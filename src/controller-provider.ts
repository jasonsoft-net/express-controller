...
export class ControllerProvider {
  ...
  public static initControllers(options: ControllerOptions): void {
    ...
    glob.sync(dist).forEach((file) => {
      ...
      if (isFunction(handler.default)) {
        ...
        console.log(
          `\x1B[33m[@jasonsoft/express-controller]\x1B[39m\x1B[32m LoadController { ${controller.name} }\x1B[39m`,
        );
  ...
  private static bindPathToRouter(
    ...
    console.log(
      `\x1B[33m[@jasonsoft/express-controller]\x1B[39m\x1B[32m RouterMapped => ${requestMethod.toUpperCase()} ${routerPath} \x1B[39m`,
    );
    router[requestMethod](routerPath, (req: any, res: any, next: any) => {
      const ctl = new controller(req, res, next);
      Promise.resolve(ctl[methodName](req, res))
        .then((result) => {
          if (isJSON(result)) {
            res.json(result);
          } else {
